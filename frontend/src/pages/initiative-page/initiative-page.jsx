...
const InitiativePage = (props) => {
  ...
  React.useEffect(() => {
    fetch(`http://172.31.3.166:8000/api/initiative/${id}`)
    .then(res => res.json())
  ...
  return (
    <Container>
      ...
      <Row className="d-flex justify-content-center">
        {data.signed} людей подписали, следующая цель: {data.required_amount_of_signatures}. всего необходимо: {data.required_amount_of_signatures}
      </Row>
      <Row className="m-auto mt-2 mb-2" style={{ width: "60%", minWidth: 300, margin: "10px auto" }}>
        <ProgressBar now={data.signed/data.required_amount_of_signatures*100} variant="danger" style={{boxSizing: "border-box", padding: 0, height: 5}}/>
      </Row>
      <Row
        style={{
          width: "60%",
